      ...
      <div className="basis-1/2">
        <section className="">
          ...
          <div className="flex flex-col flex-wrap mt-4 gap-4 sm:flex-row">
            <OverviewCard title="Total Kas" total={32_080_000} icon="total_cash.png" className="bg-secondary" />
            <OverviewCard title="Pemasukan" total={60_080_000} icon="income.png" className="bg-success" />
            <OverviewCard title="Pengeluaran" total={27_920_000} icon="outcome.png" className="bg-destructive" />
          </div>
        </section>
        ...
        <section className="bg-white p-4 mt-[1.125rem] lg:mt-9 rounded-3xl">
          ...
          <div className='flex flex-col gap-5'>
              {transactions.length === 0 ? (
                <p className='text-navy text-base font-medium text-center'>Tidak ada transaksi kas</p>
              ) : (
        ...
        <div className='flex flex-col gap-6 mt-3'>
              {announcements.length === 0 ? (
                <div className='flex items-end p-8 text-navy bg-white justify-center rounded-3xl mt-6'>
                  <p className='text-primary font-medium text-center'>Tidak ada pemberitahuan</p>
                </div>
              ) : (
                announcements.map((announcement) => (
                  <AnnouncementBox
                    ...
                    date={announcement.created_at ? new Date(announcement.created_at) : new Date()}
                  />
                ))
              )}
            </div>
